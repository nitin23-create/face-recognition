...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'facerevival@gmail.com' ")
...
def TrackImages():
    ...
    exists1 = os.path.isfile("persondetails/persondetails.csv")
    if exists1:
        df = pd.read_csv("persondetails/persondetails.csv")
    else:
        mess._show(title='Details Missing', message='Person details are missing, please check!')
        cam.release()
        cv2.destroyAllWindows()
        window.destroy()
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                bb = str(aa)
                bb = bb[2:-2]
                Data = [str(ID), '', bb, '', str(date), '', str(timeStamp)]

            else:
                Id = 'Unknown'
                bb = str(Id)
            cv2.putText(im, str(bb), (x, y + h), font, 1, (255, 255, 255), 2)
        cv2.imshow('Fetching Details', im)
        if (cv2.waitKey(1) == ord('q')):
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("Data/Data" + date + ".csv")
    if exists:
        with open("Data/Data" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
        ...
        with open("Data/Data" + date + ".csv", 'a+') as csvFile1:
            ...
            writer.writerow(Data)
        csvFile1.close()
    with open("Data/Data" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
